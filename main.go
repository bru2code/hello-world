...
// Grab inputs for processing 
	...
	fmt.Printf("gitclone %s \n", gitClone(gitPath, gheUrl, packageDir, repoName))
	fmt.Printf("gitcommit %s \n", gitCommit(gitPath, packageDir, commitMsg))
        fmt.Printf("gitpush %s \n", gitPush(gitPath))
}

func gitClone(path, gheurl, directory, repoName string) string {
...
func gitCommit(path, directory, msg string) string {
	cmd := exec.Command("cp", "-rp", directory, ".")
	stdoutStderr, err := cmd.CombinedOutput()
	fmt.Printf("%v \n", string(stdoutStderr))
	if err != nil {
		log.Fatalf("cmd.Run() failed with %s \n", err)
	...
	cmd = exec.Command(path, "commit","-a", "-m", msg)
...
func gitPush(path string) string {
	cmd := exec.Command(path, "push", "origin", "master")
	stdoutStderr, err := cmd.CombinedOutput()
	fmt.Printf("%v \n", string(stdoutStderr))
	if err != nil {
		log.Fatalf("cmd.Run() failed with %s \n", err)
